...
public class BlueToothService {
	...
	public void ScanDevice() {
		...
		SetScanState(PrinterClass.STATE_SCANING);
	...
	public void StopScan() {
		...
		adapter.cancelDiscovery();
		SetScanState(PrinterClass.STATE_SCAN_STOP);
	}
	...
	private void connectionSuccess() {
		setState(PrinterClass.STATE_CONNECTED);
		mHandler.obtainMessage(PrinterClass.MESSAGE_STATE_CHANGE, PrinterClass.SUCCESS_CONNECT, -1)
				.sendToTarget();
	}

	private void connectionFailed() {
		setState(PrinterClass.STATE_LISTEN);
		mHandler.obtainMessage(PrinterClass.MESSAGE_STATE_CHANGE, PrinterClass.FAILED_CONNECT, -1)
				.sendToTarget();
	}
	...
	private void connectionLost() {
		setState(PrinterClass.STATE_LISTEN);
		mHandler.obtainMessage(PrinterClass.MESSAGE_STATE_CHANGE, PrinterClass.LOSE_CONNECT, -1)
				.sendToTarget();
	...
	private void Nopointstart() {
		setState(PrinterClass.STATE_LISTEN);
		mHandler.obtainMessage(PrinterClass.MESSAGE_STATE_CHANGE, PrinterClass.LOSE_CONNECT, 0)
				.sendToTarget();
	...
	private class ConnectedThread extends Thread {
		 ...
		 public void run() {
	            ...
	            while (true) {
	                try {
	                    ...
	                    mHandler.obtainMessage(PrinterClass.MESSAGE_READ, bytes, -1, buffer)
	                            .sendToTarget();
	                } catch (IOException e) {
		...
		public void write(byte[] buffer) {
			try {
				...
				mHandler.obtainMessage(PrinterClass.MESSAGE_WRITE, -1, -1, buffer)
						.sendToTarget();
			} catch (IOException e) {
